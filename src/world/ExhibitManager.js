...
export class ExhibitManager {
  ...
  _buildVitrineProduct(shop) {
    const catalog = PRODUCT_CATALOG[shop.productId];
    const left = shop.side === 'left';
    const x = shop.facade.position[0] + (left ? -1 : 1) * 0; // 展台靠在店内中央偏内
    const z = shop.facade.position[2];
    ...
    const product = new ExhibitProduct(
      {
        ...
        model: `${CONFIG.assets.productRoot}${shop.productId}.glb`,
        poster: `${CONFIG.assets.posterRoot}${shop.productId}.jpg`,
  ...
  enterShop(shop) {
    ...
    // 进店站位：门内 1.0m（刚过门槛，停到展台视线前方），并转向展台
    const x = shop.door.position[0] + (left ? 1.0 : -1.0);
    const toYaw = left ? 0 : Math.PI;
  ...
  exitShop() {
    ...
    // 店外站位：街中（门外 1.2m）
    const x = shop.door.position[0] + (leftd ? 1.2 : -1.2);
